window.onload = () => {
  ...
  // ================== 지도 ====================
  // 🔹 미리보기 지도 (작은 지도)
  const previewMap = L.map('mapPreview', {
    zoomControl: false,
  ...
  // 🔹 전체 지도 (모달 내부)
  const fullMap = L.map('mapFullscreen').setView([37.5665, 126.9780], 13);
  ...
  // 🔹 모달 열기
  document.getElementById('mapPreviewContainer').addEventListener('click', () => {
